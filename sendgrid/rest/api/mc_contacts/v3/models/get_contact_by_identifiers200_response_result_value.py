...
class GetContactByIdentifiers200ResponseResultValue:
    def __init__(
        self, contact: Optional[ContactDetails3] = None, error: Optional[str] = None
    ):
    ...
    def to_dict(self):
        return {
            ...
            for key, value in {"contact": self.contact, "error": self.error}.items()
    ...
    def generate_model(payload: Dict[str, object]):
        return GetContactByIdentifiers200ResponseResultValue(
            contact=payload.get("contact"), error=payload.get("error")
        )
